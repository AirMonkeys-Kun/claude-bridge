...
:: Check all 8 heartbeats with one PowerShell call
powershell -NoProfile -Command ^
  "$base='%~dp0';" ^
  "$dirs=@('file_bridge','registry_bridge','process_bridge','network_bridge','system_bridge','wsl_bridge','user_bridge','.');" ^
  "$max=%MAX_AGE_SECONDS%;" ^
  "$stale=@();" ^
  "foreach ($d in $dirs) {" ^
...
:: Kill all V4 worker/scheduler processes
powershell -NoProfile -Command ^
  "Get-CimInstance Win32_Process -Filter \"Name='powershell.exe'\" | Where-Object { " ^
    "$_.CommandLine -like '*worker.ps1*' -or $_.CommandLine -like '*scheduler.ps1*'" ^
  "} | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue; Write-Host ('Killed PID='+$_.ProcessId) }"
timeout /t 2 /nobreak >nul

:: Clean stale artifacts
for %%d in (file_bridge registry_bridge process_bridge network_bridge system_bridge wsl_bridge user_bridge) do (
...
start /B powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0registry_bridge\worker.ps1" -WorkerDir "%~dp0registry_bridge"
start /B powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0process_bridge\worker.ps1" -WorkerDir "%~dp0process_bridge"
start /B powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0network_bridge\worker.ps1" -WorkerDir "%~dp0network_bridge"
