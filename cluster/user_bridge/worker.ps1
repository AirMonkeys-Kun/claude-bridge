...
$null = $filePs.AddScript({
    ...
    TLogf "User file monitor started"; $lastId=""; $contentCache=@{}
    ...
    while ($true) {
        ...
        if ($q -and $q.state -eq "pending" -and $q.cmd_id -ne "" -and $q.cmd_id -ne $lastId) {
            # V13: Content-hash dedup — skip if same command text ran recently
            $raw=$q.command
            if ($raw -and $raw.Length -gt 0) {
                ...
                if ($contentCache.ContainsKey($contentKey)) {
                    ...
                    if ($cachedAge -lt 120000) {
                        TLogf "  [$($q.cmd_id)] CONTENT DEDUP HIT — same command ran $($cachedAge)ms ago, skipping"
